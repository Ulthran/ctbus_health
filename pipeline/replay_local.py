#!/usr/bin/env python3
"""
Local replay of the food processor pipeline using Amazon Bedrock.
Reads raw Telegram messages from S3, groups them by proximity, processes with the model.
Output: ./output/diet/{date}.json

Usage:
    python pipeline/replay_local.py
"""
import hashlib
import json
...
OUTPUT_DIR = Path("output/diet")
RESTAURANT_CACHE_DIR = Path("output/cache/restaurant")
...
def main():
    ...
    if not _PYZBAR:
        print("WARNING: pyzbar not installed — barcode photos will not be decoded.")
        print("         Install with: sudo apt-get install -y libzbar0 && pip install pyzbar Pillow\n")
    print("Fetching raw messages from S3...")
    messages = _load_all_messages()
    messages.sort(key=lambda m: m["timestamp"])
    print(f"Loaded {len(messages)} messages")

    groups = _group_by_proximity(messages)
    print(f"Grouped into {len(groups)} entries\n")
    ...
    for i, group in enumerate(groups):
        ...
        print(f"[{i+1}/{len(groups)}] message_ids={ids}")
        for t in texts:
            print(f"  text: {t[:80]}")
        if photo_keys:
            print(f"  photos: {photo_keys}")
        ...
        _history_cache = None  # refresh so next group sees entries written so far
        print()
...
def _load_all_messages() -> list[dict]:
    ...
    for page in paginator.paginate(Bucket=BUCKET, Prefix="raw/telegram/"):
        for obj in page.get("Contents", []):
            ...
            # Photos were stored under UTC dates before the ET fix
            photo_s3_key = None
            if "photo" in msg:
                for candidate in [
                    f"photos/{utc_date}/{message_id}.jpg",
                    f"photos/{utc_dt.astimezone(ET).date().isoformat()}/{message_id}.jpg",
                ]:
                    try:
                        s3.head_object(Bucket=BUCKET, Key=candidate)
                        photo_s3_key = candidate
                        break
                    except Exception:
                        pass
...
def _process_group(group: list[dict]):
    ...
    out_path = OUTPUT_DIR / f"{et_date}.json"
    daily = json.loads(out_path.read_text()) if out_path.exists() else {"date": et_date, "entries": []}
    ...
    for key in photo_keys:
        photo_bytes = s3.get_object(Bucket=BUCKET, Key=key)["Body"].read()
    ...
    if photo_keys and not texts and not barcode_data:
        texts = [f"{len(photo_keys)} barcode photo(s) — barcode unreadable, estimate from context or call lookup_openfoodfacts"]

    components = _extract_components(texts, barcode_data)
    ...
    total_cal = sum(c.get("nutrition_total", {}).get("calories_kcal") or 0 for c in components)
    print(f"  -> {len(components)} components, ~{total_cal:.0f} kcal -> {out_path}")
...
def _extract_barcodes(photo_bytes: bytes) -> list[str]:
    ...
    try:
        ...
        img = _PILImage.open(_io.BytesIO(photo_bytes))
...
# ── history context (reads local output/diet/ files) ─────────────────────────
...
def _get_history() -> list[dict]:
    ...
    for i in range(7):
        path = OUTPUT_DIR / f"{(today - timedelta(days=i)).isoformat()}.json"
        if path.exists():
...
def _get_restaurant_cache(restaurant: str, location: str) -> dict | None:
    cache_file = RESTAURANT_CACHE_DIR / f"{_restaurant_cache_key(restaurant, location)}.json"
    try:
...
def _set_restaurant_cache(restaurant: str, location: str, result: dict):
    RESTAURANT_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache_file = RESTAURANT_CACHE_DIR / f"{_restaurant_cache_key(restaurant, location)}.json"
    cache_file.write_text(json.dumps({"cached_at": datetime.now(UTC).isoformat(), "result": result}, indent=2))


def _search_restaurant_menu(restaurant_name: str, location: str, items: list[str]) -> dict:
    if not TAVILY_API_KEY:
        return {"found": False, "error": "TAVILY_API_KEY not set in replay_local.py"}
...
def _extract_components(texts: list[str], barcode_data: list[dict] | None = None) -> list[dict]:
